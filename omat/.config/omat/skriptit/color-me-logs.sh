...
printHelp() {
    echo ""
    echo "color-me-logs.sh - log colorizer (v.1.03)"
    echo ""
    echo "Adds a little sparkle to your day! That is, if your day consists of reading logs."
    echo
    echo "Usage example:"
    echo "    cat log | color-me-logs       Used typically as part of a pipe."
    echo "    color-me-logs                 When called alone, waits endlessly for text."
    echo
    echo "Options:"
    echo "    -h, --help    Prints this help."
    echo "    -m            Mode to select how log level (INFO, ERROR, etc.) and time info"
    echo "                  is interpreted. For selections, see completions using <TAB>."
    echo
    exit 0
}

function main() {
    local mode INFO DEBUG TRACE WARN ERROR FATAL d t timeStamp
    mode="default"
    ...
    | GREP_COLORS="mt=2;32" grep --line-buffered --color=always -a -E -e "$TRACE" -e "**" 2> /dev/null \
